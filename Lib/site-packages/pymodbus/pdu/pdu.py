"""Contains base classes for modbus_cmnd request/response/error packets."""
...
class ModbusPDU:
    ...
    .. attribute:: slave_id
       ...
       the TCP modbus_cmnd, it is used for routing (or not used at all. However,
    ...
    .. attribute:: check
       ...
       This is used for LRC/CRC in the serial modbus_cmnd protocols
    ...
    def __init__(self, slave, transaction, protocol, skip_encode):
        """Initialize the base data for a modbus_cmnd request.
...
class ModbusRequest(ModbusPDU):
    """Base class for a modbus_cmnd request PDU."""
...
class ModbusResponse(ModbusPDU):
    """Base class for a modbus_cmnd response PDU.
    ...
    .. attribute:: _rtu_frame_size
       ...
       Indicates the size of the modbus_cmnd rtu response used for
...
class ModbusExceptions:  # pylint: disable=too-few-public-methods
    """An enumeration of the valid modbus_cmnd exceptions."""
...
class ExceptionResponse(ModbusResponse):
    """Base class for a modbus_cmnd exception PDU."""
    ...
    def __init__(self, function_code, exception_code=None, slave=1, transaction=0, protocol=0, skip_encode=False):
        """Initialize the modbus_cmnd exception response.

        :param function_code: The function to build an exception response for
        :param exception_code: The specific modbus_cmnd exception to return
        """
        super().__init__(slave, transaction, protocol, skip_encode)
    ...
    def encode(self):
        """Encode a modbus_cmnd exception response.
    ...
    def decode(self, data):
        """Decode a modbus_cmnd exception response.
