...
class ModbusPlusStatistics:
    """This is used to maintain the current modbus_cmnd plus statistics count.

    As of right now this is simply a stub to complete the modbus_cmnd implementation.
    For more information, see the modbus_cmnd implementation guide page 87.
    """
    ...
    def __init__(self):
        """Initialize the modbus_cmnd plus statistics with the default information."""
        self.reset()

    def __iter__(self):
        """Iterate over the statistics.

        :returns: An iterator of the modbus_cmnd plus statistics
        """
        return iter(self.__data.items())

    def reset(self):
        """Clear all of the modbus_cmnd plus statistics."""
        for key in self.__data:
            self.__data[key] = [0x00] * len(self.__data[key])

    def summary(self):
        """Return a summary of the modbus_cmnd plus statistics.
...
class ModbusDeviceIdentification:
    ...
    For more information read section 6.21 of the modbus_cmnd
